...
public class BasketPage extends BasePage {
    ...
    public int getPriceOfProductInBasket(String articl) {

        for (WebElement product : shoppingList) {
            if (product.findElement(By.xpath(".//div[@class='cart-items__product-code']/div"))
                    .getText().contains(articl)) {
                return Integer.parseInt(waitUntilElementToBeVisible(product.findElement(By.xpath(".//span[@class='price__current']")))
    ...
    public BasketPage countOfProductInBasket(String articl) {
        for (WebElement product : shoppingList) {
            scrollWithOffset(product, 0, -250);
            if (product.findElement(By.xpath(".//div[@class='cart-items__product-code']/div"))
                    .getText().contains(articl)) {
                try {
                    Thread.sleep(2000);
